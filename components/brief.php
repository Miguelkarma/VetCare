<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
 <link rel="stylesheet"  href="../../ovas/components/css/brief.css">


<div class="container brief-container">
  <div class="row with-gap align-items-center ">
    <!-- Text Content -->
    <div class="col-lg-12 col-md-12 ">
        <div class="light">
      <div class="text-content ">
        <span class="text-uppercase font-weight-semibold">Explore More</span>
        <h2 class="title display-4 mt-4 mb-3 font-weight-bold">Everything You Need in One Place</h2>
        <h5 class="mb-4">We care for your pets like family. Here's what you can do with us:</h5>

      
<div class="d-flex mb-4">
  <div>
    <i class="fas fa-users h4 mt-1"></i>
  </div>
  <div class="ml-3">
    <h4 class="mb-1">About Us</h4>
    <p class="mb-2">
      Get to know our passionate team of animal lovers and professionals committed to providing
      the highest level of care for your pets. We believe every pet deserves love, attention, and
      the best veterinary support, and we’re proud to make that our mission every day.
    </p>
  <a href="http://localhost/ovas/?page=about" class="learn-btn">
  <div class="text">
    <span>Learn More</span>
  </div>
  <div class="clone">
    <span>Learn More</span>
  </div>
<i class="fas fa-arrow-circle-right" style="margin-right:0.4em;"></i>
</a>
  </div>
</div>


<div class="d-flex mb-4">
  <div>
    <i class="fas fa-syringe h4 mt-1"></i>
  </div>
  <div class="ml-3">
    <h4 class="mb-1">Our Services</h4>
    <p class="mb-2">
      From routine check-ups and vaccinations to advanced diagnostics and emergency interventions, 
      our clinic offers a full range of veterinary services tailored to your pet’s needs. 
      We prioritize comfort, safety, and personalized care in every visit.
    </p>
     <a href="http://localhost/ovas/?page=services"  class="learn-btn">
  <div class="text">
    <span>See More</span>
  </div>
  <div class="clone">
    <span>See More</span>
  </div>
<i class="fas fa-arrow-circle-right" style="margin-right:1.1em;"></i>
</a>
  </div>
</div>


<div class="d-flex">
  <div>
    <i class="fas fa-envelope  h4 mt-1"></i>
  </div>
  <div class="ml-3">
    <h4 class="mb-1">Contact Us</h4>
    <p class="mb-2">
      Whether you’re scheduling a visit, asking questions about your pet’s health, 
      or looking for advice on how to care for your furry companion, we’re here to help. 
      Reach out via email, phone, or stop by our clinic — we’re always happy to hear from you!
    </p>
     <a href="http://localhost/ovas/?page=contact_us" class="learn-btn">
  <div class="text">
    <span>Get in Touch</span>
  </div>
  <div class="clone">
    <span>Get in Touch</span>
  </div>
<i class="fas fa-arrow-circle-right" style="margin-left:0.8em;"></i>
</a>
  </div>
</div>
</div>
  </div>
</div>