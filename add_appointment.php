...
<div class="container-fluid">
        ...
        <div class="row">
            ...
            <?php if(!empty($_settings->userdata('username'))){ ?>
                <fieldset>
                    ...
                    <select id="user-select" style="width: 300px;"></select>
                    </div>
                    <div class="form-group">
                    <label for="breed" class="control-label">Pet Type</label>
                    <input type="hidden" name="pet_id" id="pet_id" class="form-control form-control-border" placeholder="Siberian Husky" value ="<?php echo isset($pet_id) ? $pet_id : '' ?>" required>
                ...
                <fieldset>
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="pet_id" id="pet_id" class="form-control form-control-border" placeholder="Siberian Husky" value ="0" required>
                        <select name="category_id" id="category_id" class="form-control form-control-border select2">
                            <option value="" selected disabled></option>
                            <?php 
                            $categories = $conn->query("SELECT * FROM category_list where delete_flag = 0 ".(isset($category_id) && !empty($category_id) ? " or id = '{$category_id}'" : "")." order by name asc");
    ...
    <script>
        ...
        $('#user-select').on('select2:select', function(e) {
            ...
            $('#selected-name').val(selectedData.text); // User name
            $('#category_id').val(selectedData.id);  
            $('#breed').val(selectedData.breed);
            $('#age').val(selectedData.age);
            $('#pet_id').val(selectedData.pet_id);
            $('#category_id').val(selectedData.id).trigger('change');  // User ID
        ...
        $('#add-user-form').submit(function(event) {
            ...
            $.ajax({
                url: 'db_helper.php?action=add_user',
                method: 'POST',
                data: formData,
                success: function(response) {
                    alert(response.success || response.error);
                    // Reload Select2 dropdown
