...
select option:checked {
    background-color: #f1d2bd !important;
    color: rgb(75, 49, 12) !important;
    background: #f1d2bd !important;
}
select option:hover{
    background-color: #f1d2bd !important;
}
            ...
            <div class="col-md-6">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            ...
                            <select id="user-select" class="form-control">
                                <option value="" selected disabled>Search for a pet</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" id="category_id" class="form-control">
                                <option value="" selected disabled>Please Select Pet Type Here</option>
                                <?php 
                ...
                <div class="form-group">
                    ...
                    <select name="service_id" id="service_id" class="form-control">
                        <option value="" selected disabled>Please Select Service Here</option>
                    </select>
                </div>
...
<script>
    ...
    $('#user-select').on('change', function() {
        $.ajax({
            url: 'db_helper.php?action=get_pet_data',
            type: 'GET',
            dataType: 'json',
            data: { pet_id: $(this).val() },
    ...
    <?php if(!empty($_settings->userdata('username'))) { ?>
    $.ajax({
        url: 'db_helper.php?action=get_user_pets',
        type: 'GET',
        dataType: 'json',
        ...
        success: function(data) {
            if(data && data.length > 0) {
                $('#user-select').empty();
                $('#user-select').append('<option value="" selected disabled>Select a pet</option>');
                
                $.each(data, function(i, pet) {
                    $('#user-select').append('<option value="' + pet.id + '">' + pet.name + '</option>');
                });
            }
        }
    });
    ...
    $(function(){
        ...
        $('#category_id').change(function(){
            ...
            var placeholderOpt = $("<option selected disabled>").text("Please Select Service Here");
            $('#service_id').append(placeholderOpt);
            
            // Add service options that match the selected category
